import type { experience_category, experience_type, user_experience } from '@prisma/client';
...
const expPoints =
  env.NODE_ENV === 'production' ? Math.floor(Math.random() * (25 - 15 + 1)) + 15 : 30;
...
export async function awardGitHubXP(
  ...
  // Calculate level ups
  const levelUpResult = await calculateLevelUps(experienceData.level, newLevelPoints);
...
export async function experience(
  ...
  if (!experienceData) {
    ...
    if (type === 'TEXT') {
      ...
      const role = (await channel.guild?.roles.fetch(env.ROLE_VIP_0))!;
  ...
  if (experienceData.last_message_at.getTime() + expInterval > Date.now()) {
  ...
  if (expToLevel <= experienceData.level_points) {
    ...
    const channelTripbotLogs = (await channel.guild.channels.fetch(
      env.CHANNEL_BOTLOG,
    )) as TextChannel;
    await channelTripbotLogs.send(
      stripIndents`${member.displayName} has leveled up to ${categoryName} ${typeName} level ${experienceData.level}!`,
    );
    ...
    if (experienceData.level % 10 === 0) {
      ...
      switch (category) {
        case 'DEVELOPER' as experience_category: {
          id = env.CHANNEL_DEVELOPMENT;

          break;
        }
        case 'GENERAL' as experience_category: {
          id = env.CHANNEL_LOUNGE;

          break;
        }
        case 'IGNORED' as experience_category: {
          id = env.CHANNEL_BOTSPAM;

          break;
        }
        case 'TEAM' as experience_category: {
          id = env.CHANNEL_TEAMTRIPSIT;

          break;
        }
        case 'TRIPSITTER' as experience_category: {
...
export async function findXPfromLevel(level: number): Promise<number> {
  ...
  totalXP = xpResults.reduce((accumulator, xp) => accumulator + xp, 0);
...
export async function getTotalLevel(
  ...
  while (levelPoints >= expToLevel) {
    ...
    const newExpToLevel = await expForNextLevel(level);
...
export async function giveMilestone(member: GuildMember) {
  const userData = await db.users.upsert({
    create: {
      discord_id: member.id,
    },
  ...
  const role = (await member.guild?.roles.fetch(
    roleDefs[levelTier as keyof typeof roleDefs].role,
  ))!;
  ...
  if (levelTier >= 1) {
    ...
    const previousRoles = Object.keys(roleDefs)
      ...
      .map((key) => roleDefs[Number.parseInt(key, 10) as keyof typeof roleDefs].role);

    // Remove all previous roles
    previousRoles.forEach(async (previousRole) => {
      const removeRole = (await member.guild?.roles.fetch(previousRole))!;
  ...
  if (!member.roles.cache.has(role.id)) {
    ...
    if (levelTier >= 2) {
      ...
      await channel.send(
        `${emojis} **${member} has reached Total level ${levelTier}0!** ${emojis}`,
      );
    }
  }
}
...
): Promise<{ finalLevel: number; levelsGained: number; remainingPoints: number }> {
...
async function sendGitHubXPNotifications(
  ...
  const milestonePromises = milestoneLevels.map(async (level) =>
    sendMilestoneAnnouncement(member, channel.id, level, categoryName, typeName),
  );
