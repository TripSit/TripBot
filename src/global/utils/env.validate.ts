...
export default function validateEnvironment(
  service: 'DISCORD' | 'IRC' | 'MATRIX' | 'SERVICES' | 'TELEGRAM',
) {
  ...
  if (service === 'TELEGRAM' && !process.env.TELEGRAM_TOKEN) {
    log.error(
      F,
      stripIndents`Missing TELEGRAM_TOKEN: You wont be able to login to Telegram. \
  ...
  if (service === 'DISCORD') {
    if (!process.env.DISCORD_GUILD_ID) {
      log.error(
        F,
        stripIndents`Missing DISCORD_GUILD_ID: You get this from your discord guild. \
    ...
    if (!process.env.DISCORD_OWNER_ID) {
      log.warn(
        F,
        stripIndents`Missing DISCORD_OWNER_ID: Certain commands wont respond right, but nbd. \
    ...
    if (!process.env.DISCORD_CLIENT_ID) {
      log.error(
        F,
        stripIndents`Missing DISCORD_CLIENT_ID: You wont be able to login to discord. \
    ...
    if (!process.env.DISCORD_CLIENT_TOKEN) {
      log.error(
        F,
        stripIndents`Missing DISCORD_CLIENT_TOKEN: You wont be able to login to discord.\
       You get this from the discord developer portal.`,
      );
      return false;
    }
    ...
    if (!process.env.PRISMA_DB_URL) {
      log.error(
        F,
        stripIndents`Missing PRISMA_DB_URL: You wont be able to use the database via Prisma.\
    ...
    if (process.env.DISCORD_CLIENT_TOKEN === 'In your Discord Developer Portal') {
      log.error(
        F,
        `Welcome to TripBot. This is likely your first run, congrats on making it this far!
