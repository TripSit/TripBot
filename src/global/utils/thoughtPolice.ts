...
export async function bigBrother(messageContent: string): Promise<string> {
  ...
  if (offensive.some((pattern) => new RegExp(pattern).test(messageContent))) {
    return 'offensive';
  }

  // Check for HR alerts next
  if (harmReduction.some((pattern) => new RegExp(pattern).test(messageContent))) {
    return 'harm';
  }

  // Check for horny people next
  if (hornyJail.some((pattern) => new RegExp(pattern).test(messageContent))) {
    return 'horny';
  }

  // Check for uncouth language
  if (pg13.some((pattern) => new RegExp(pattern).test(messageContent))) {
    return 'pg13';
  }

  // Check for memes next
  if (memes.some((pattern) => new RegExp(pattern).test(messageContent))) {
    return 'meme';
  }
