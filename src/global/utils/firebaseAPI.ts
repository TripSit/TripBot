...
export async function firebaseConnect(): Promise<void> {
  ...
  //   provider: new ReCaptchaV3Provider(FIREBASE_REALTIME_KEY),
  ...
  const userRef = global.db.ref(`${env.FIREBASE_DB_USERS}`);
  ...
  // GUILD
  const guildRef = global.db.ref(`${env.FIREBASE_DB_GUILDS}`);
...
// export async function getUserInfo(member: any): Promise<void> {
...
//       const ref = db.ref(`${FIREBASE_DB_GUILDS}/${guild.name.toString()}`);
//       // logger.debug(`[${PREFIX}] ref: ${ref}`);
//       await ref.once('value', data => {
//         if (data.val() !== null) {
//           // logger.debug(`[${PREFIX}] data: ${JSON.stringify(data.val(), null, 2)}`);
...
//       guildData.guild_owner_id = guild.DISCORD_OWNER_ID || 'No Owner';
...
//     if (db !== undefined) {
//       db.ref(FIREBASE_DB_GUILDS).update({
//         [id]: data,
//       });
//     }
//   },
...
//     if (db !== undefined) {
//       const ref = db.ref(`${FIREBASE_DB_TICKETS}/${id}`);
//       logger.debug(`[${PREFIX}] ref: ${ref}`);
//       await ref.once('value', data => {
//         if (data.val() !== null) {
...
//       try {
//         await db.ref(FIREBASE_DB_TICKETS).update({
//           [fbid]: data,
//         });
