...
export default async function webserverConnect(): Promise<void> {
  ...
  if (env.NODE_ENV === 'development') {
    ...
    log.info(F, `Webserver running at: https://${host}:${httpsPort}/`);
    // httpsServer.listen(httpsPort, () => {
  ...
  app.post('/user', (req, res) => {
    ...
    log.debug(F, `codeValue: ${codeValue}`);
    log.debug(F, `url: ${redirectUrl}`);
    ...
    fetch('https://discord.com/api/oauth2/token', { method: 'POST', body: data1 }).then(response => response.json()).then((data:any) => {
      ...
      axios.get('https://discord.com/api/users/@me', makeConfig(data.access_token)).then(response => {
        ...
        // log.debug(F, `response: ${JSON.stringify(response)}`);
        ...
        log.error(F, `${JSON.stringify(err, null, 2)}`); // Log the error in the console
