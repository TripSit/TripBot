...
export const emojiUpdate: emojiUpdateEvent = {
  ...
  async execute(oldEmoji, newEmoji) {
    ...
    if (!creationLog) {
      botlog.send(`${newEmoji.name} was updated, but no relevant audit logs were found.`);
      return;
    }
    ...
    if (creationLog.executor) {
      response = `${newEmoji.name} was updated by ${creationLog.executor.tag}:`;
      response = `${creationLog.changes.map((change) => `\`${change.key}\` changed from \`${change.old}\` to \`${change.new}\``).join('\n')}`; // eslint-disable-line max-len
    } else {
      response = `${newEmoji.name} was updated, but the audit log was inconclusive.`;
      response = `${creationLog.changes.map((change) => `\`${change.key}\` changed from \`${change.old}\` to \`${change.new}\``).join('\n')}`; // eslint-disable-line max-len
    }
