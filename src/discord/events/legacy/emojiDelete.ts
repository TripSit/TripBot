...
export const emojiDelete: EmojiDeleteEvent = {
  ...
  async execute(emoji) {
    ...
    if (perms.length > 0) {
      ...
      await guildOwner.send({ content: `Please make sure I can ${perms.join(', ')} in ${emoji.guild} so I can run ${F}!` }); // eslint-disable-line
      log.error(F, `Missing permission ${perms.join(', ')} in ${emoji.guild}!`);
      return;
    }
