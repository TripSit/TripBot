...
export const roleCreate: RoleCreateEvent = {
  ...
  async execute(role) {
    ...
    if (perms.length > 0) {
      ...
      await guildOwner.send({ content: `Please make sure I can ${perms.join(', ')} in ${role.guild} so I can run ${F}!` }); // eslint-disable-line
      log.error(F, `Missing permission ${perms.join(', ')} in ${role.guild}!`);
      return;
    }
