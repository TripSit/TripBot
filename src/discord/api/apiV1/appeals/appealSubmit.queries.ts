...
async function getModThread(
  ...
  if (!userData) {
    log.error(F, `Could not find user with id ${userId}`);
...
export default {
  async handle(
    ...
    //         res.status(418).send('Thank you for asking, your appeal is still undecided,
    // and it\'s been over a week so I sent a reminder to the guild.');
    //         return;
    //       }

    //       res.status(418).send('Thank you for asking, you have an existing appeal
    // that is still open. Please be patient.');
    //       return;
    //     }
    ...
    const actorString = banLog?.executor
      ? `${banLog.executor.username}#${banLog.executor.discriminator}`
      : 'unknown member';
