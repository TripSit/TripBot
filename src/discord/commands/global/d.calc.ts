import type { ChatInputCommandInteraction, ColorResolvable, EmbedBuilder } from 'discord.js';

import { stripIndents } from 'common-tags';
import { Colors, MessageFlags, SlashCommandBuilder } from 'discord.js';

import type { DxmDataType } from '../../../global/commands/g.calc';
...
const invalidParametersErrorMessage = 'Invalid values supplied';
const ephemeralMessage = 'Set to "True" to show the response only to you';
const askWeightMessage = 'How much do you weigh?';
 ...
 */
async function buildCalcEmbed(
  title: string,
  description: string,
  color: ColorResolvable = Colors.Purple,
  isError = false,
  ...
  return embedTemplate()
    ...
    .setDescription(
      isError
        ? stripIndents`There was an error during conversion!
 ...
 * @returns {Promise<EmbedBuilder>}
 */
async function dCalcBenzo(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
  ...
  if (dosage < 0.001) {
    return buildCalcEmbed(
      ...
      "The parameter 'i_have' cannot be less than 0.001.",
      Colors.Red,
    );
  }
 ...
 * @returns {Promise<EmbedBuilder>}
 */
async function dCalcDXM(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
  ...
  if (givenWeight < 1) {
    return buildCalcEmbed(
      invalidParametersErrorMessage,
      "The parameter 'calc_weight' cannot be less than 1.",
      Colors.Red,
    );
  }
  ...
  let header = true;
  for (const key of Object.keys(dosageData)) {
    embed.addFields(
      { inline: true, name: header ? 'Plateau' : '\u200B', value: `**${key}**` },
      {
        inline: true,
        name: header ? 'Minimum' : '\u200B',
        value: `${dosageData[key as keyof DxmDataType].min} ${units}`,
      },
      {
        inline: true,
        name: header ? 'Maximum' : '\u200B',
        value: `${dosageData[key as keyof DxmDataType].max} ${units}`,
      },
    );
    header = false;
  }
 ...
 * @returns {Promise<EmbedBuilder>}
 */
async function dCalcKetamine(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
 ...
 * @returns {Promise<EmbedBuilder>}
 */
async function dCalcMDMA(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
  const givenWeight = interaction.options.getNumber('weight', true);

  if (givenWeight < 1) {
    return buildCalcEmbed(
      invalidParametersErrorMessage,
      "The parameter 'weight' cannot be less than 1.",
      Colors.Red,
    );
  }
  const weightUnits = interaction.options.getString('units', true) as 'kg' | 'lbs';
 ...
 * @returns {Promise<EmbedBuilder>}
 */
async function dCalcNasal(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
  ...
  const imageUrl =
    'https://user-images.githubusercontent.com/1836049/218758611-c84f1e34-0f5b-43ac-90da-bd89b028f131.png';
  const embed = embedTemplate().setTitle('Nasal spray calculator').setImage(imageUrl);
  ...
  if (calculationType === 'solvent') {
    ...
    embed.setDescription(
      `You'll need ~${await calcSolvent(amount, desiredMgPerPush, mlPerPush)}ml of solvent (water)`,
    );
    ...
    // log.debug(F, `ml_per_push: ${mlPerPush}`);
    embed.setDescription(
      `You'll need ~${await calcSubstance(amount, desiredMgPerPush, mlPerPush)}mg of the substance`,
    );
  }
  return embed;
}
 ...
 * @returns {Promise<EmbedBuilder>}
 */
async function dCalcPsychedelics(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
  ...
  if (days < 1 || lastDose < 1 || desiredDose < 1) {
    return buildCalcEmbed(
      invalidParametersErrorMessage,
      "The parameters 'last_dose_amount', 'desired_dose_amount', and 'days' cannot be less than 1.",
      Colors.Red,
    );
  }
  ...
  title = desiredDose
    ? `${title} ${desiredDose} ${units} of ${drug} when ${lastDose} ${units} were taken ${days} days ago.`
    : `${title} ${lastDose} ${units} of ${drug} taken ${days} days ago.`;
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addNumberOption((option) =>
          option.setName('i_have').setDescription('mg').setRequired(true),
        )
        ...
        .addStringOption((option) =>
          option
            .setName('and_i_want_the_dose_of')
            .setDescription('Pick the second drug')
            .setAutocomplete(true)
            .setRequired(true),
        )
        .addBooleanOption((option) => option.setName('ephemeral').setDescription(ephemeralMessage)),
    )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addNumberOption((option) =>
          option.setName('calc_weight').setDescription(askWeightMessage).setRequired(true),
        )
        .addStringOption((option) =>
          option
            .setName('units')
            .setDescription('In what units?')
            .addChoices({ name: 'kg', value: 'kg' }, { name: 'lbs', value: 'lbs' })
            .setRequired(true),
        )
        .addStringOption((option) =>
          option
            ...
            .setDescription(
              'What are you taking? All products (except RoboTablets) contain DXM hBr.',
            )
            .addChoices(
              {
                name: 'RoboTablets (30 mg freebase tablets)',
                value: 'RoboTablets (30 mg tablets)',
              },
              ...
              {
                name: 'Robitussin Gelcaps (15 mg caps HBr)',
                value: 'Robitussin Gelcaps (15 mg caps)',
              },
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addNumberOption((option) =>
          option.setName('weight').setDescription(askWeightMessage).setRequired(true),
        )
        .addStringOption((option) =>
          option
            .setName('units')
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addNumberOption((option) =>
          option.setName('amount').setDescription('Amount of solvent in ml').setRequired(true),
        )
        .addNumberOption((option) =>
          option
            .setName('desired_mg_per_push')
      ...
      subcommand
        ...
        .addStringOption((option) =>
          option
            ...
            .setRequired(true)
            .addChoices({ name: 'LSD', value: 'lsd' }, { name: 'Mushrooms', value: 'mushrooms' }),
        )
        .addNumberOption((option) =>
          option
            .setName('last_dose_amount')
