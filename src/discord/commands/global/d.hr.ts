...
export const dHR = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub => sub
      ...
      .addBooleanOption(option => option.setName('ephemeral').setDescription('Show only to you')))
    .addSubcommand(sub => sub
      .setName('combochart')
    ...
    .addSubcommand(sub => sub
      ...
      .addBooleanOption(option => option.setName('ephemeral').setDescription('Show only to you'))) as SlashCommandBuilder,
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'combochart') {
      const chart = await combochart();
      try {
        await interaction.editReply({ content: chart });
      } catch (error) {
        await interaction.deleteReply();
        await interaction.followUp({
          content: chart,
    ...
    if (subcommand === 'breathe') {
      ...
      const data = await breathe(exercise);
      try {
    ...
    if (subcommand === 'grounding') {
      const info = await grounding();
      try {
    ...
    if (subcommand === 'recovery') {
      ...
      try {
        ...
        await interaction.deleteReply();
        await interaction.followUp({ content: info, flags: MessageFlags.Ephemeral });
      }
      return true;
    }

    if (subcommand === 'reagents') {
      const info = await reagents();
      try {
        await interaction.editReply({ content: info });
      } catch (error) {
        await interaction.deleteReply();
    ...
    if (subcommand === 'mushroom_info') {
      ...
      const reply: InteractionReplyOptions = { embeds: [embed], files, components };
      try {
