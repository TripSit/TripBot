...
export const dImgur: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('search').setDescription('What are you looking for?').setRequired(true),
    )
    .addStringOption((option) =>
      option
        .setName('sort')
  ...
  async execute(interaction) {
    ...
    const windowString = `${window}/`;

    const query = `https://api.imgur.com/3/gallery/search/${sortString}${windowString}?q=${search}`;
