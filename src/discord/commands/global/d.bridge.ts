...
async function confirm(interaction: ChatInputCommandInteraction): Promise<string> {
  // Check if the member doing this command has permissions to make channels
  if (
    !(interaction.member as GuildMember).permissions.has('ManageChannels' as PermissionResolvable)
  ) {
  ...
  const bridgedChannel = await bridgeConfirm(externalChannel.id);
  ...
  await internalChannel.send({
    embeds: [
      embedTemplate().setTitle('Bridge').setDescription(stripIndents`
...
async function create(interaction: ChatInputCommandInteraction): Promise<string> {
  // Check if the member doing this command has permissions to make channels
  if (
    !(interaction.member as GuildMember).permissions.has('ManageChannels' as PermissionResolvable)
  ) {
  ...
  if (!internalChannelPerms.hasPermission) {
    log.error(
      F,
      stripIndents`Missing ${internalChannelPerms.permission} permission \
...
async function info(interaction: ChatInputCommandInteraction): Promise<string> {
  // Check if the member doing this command has permissions to make channels
  if (
    !(interaction.member as GuildMember).permissions.has('ManageChannels' as PermissionResolvable)
  ) {
...
async function pause(interaction: ChatInputCommandInteraction): Promise<string> {
  // Check if the member doing this command has permissions to make channels
  if (
    !(interaction.member as GuildMember).permissions.has('ManageChannels' as PermissionResolvable)
  ) {
  ...
  if (interaction.guild?.id === env.DISCORD_GUILD_ID) {
    ...
    await Promise.allSettled(
      activeBridges.map(async (bridge) => bridgePause(bridge.external_channel)),
    );
...
async function remove(interaction: ChatInputCommandInteraction): Promise<string> {
  // Check if the member doing this command has permissions to make channels
  if (
    !(interaction.member as GuildMember).permissions.has('ManageChannels' as PermissionResolvable)
  ) {
  ...
  if (interaction.guild?.id === env.DISCORD_GUILD_ID) {
    ...
    await Promise.allSettled(bridges.map(async (bridge) => bridgeRemove(bridge.external_channel)));
...
async function resume(interaction: ChatInputCommandInteraction): Promise<string> {
  // Check if the member doing this command has permissions to make channels
  if (
    !(interaction.member as GuildMember).permissions.has('ManageChannels' as PermissionResolvable)
  ) {
  ...
  if (interaction.guild?.id === env.DISCORD_GUILD_ID) {
    ...
    // Resume all bridges
    await Promise.allSettled(
      activeBridges.map(async (bridge) => bridgeResume(bridge.external_channel)),
    );
...
export const dBridge: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName('external_channel')
            .setDescription('Channel ID on the other guild')
            .setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName('override').setDescription('Redo an existing bridge').setRequired(false),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('pause').setDescription('Pause a bridge between two discord channels'),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('resume').setDescription('Resume a bridge between two discord channels'),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('info').setDescription('Get info on this bridge setup'),
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName('remove')
        ...
        .addStringOption((option) =>
          option.setName('confirmation').setDescription('Confirmation Code'),
        ),
    ),
  async execute(interaction) {
    log.info(F, await commandContext(interaction));
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const embed = embedTemplate().setTitle('Bridge').setColor(Colors.DarkPurple);
    if (!interaction.guild || !interaction.member) {
      await interaction.editReply({
        embeds: [
          embed.setDescription('This command can only be used in a guild.').setColor(Colors.Red),
        ],
      });
      return false;
    }
    ...
    if (
      ...
      await interaction.editReply({
        embeds: [
          embed
            .setDescription(
              `This command can only be used in a partner guild!
