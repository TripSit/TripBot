...
import { help, link, profile, unlink } from '../../../global/commands/g.learn';
...
async function moodleHelp(): Promise<EmbedBuilder> {
  ...
  return embedTemplate().setTitle(helpData.title).setDescription(helpData.description).setFooter({
    text: helpData.footer,
  });
}

async function moodleLink(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
  ...
  if (interaction.options.getString('discord_id')) {
    ...
    return embedTemplate().setDescription(
      await link(
  ...
  return embedTemplate().setDescription(
    await link(interaction.options.getString('email', true), interaction.user.id),
  );
}

async function moodleProfile(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
  ...
  if (targets.length > 1) {
    return embedTemplate()
      .setColor(Colors.Red)
      .setTitle('Found more than one user with with that value!').setDescription(stripIndents`
  ...
  if (targets.length === 0) {
    return embedTemplate().setColor(Colors.Red).setTitle('Found no users with that value!')
  ...
  if (!discordMember) {
    return new EmbedBuilder().setDescription('Member not found!');
  }
  ...
  const completedCourses = moodleProfileData.completedCourses.join('\n *');

  // Make a string that says 'this user is enrolled in the following courses:
  // course1, course2, course3'
  const incompleteCourses = moodleProfileData.incompleteCourses.join('\n* ');

  const title = ` - ${moodleProfileData.department} of ${moodleProfileData.institution}\n`;

  const profileEmbed = new EmbedBuilder().setAuthor({
...
async function moodleUnlink(interaction: ChatInputCommandInteraction): Promise<EmbedBuilder> {
  if (interaction.options.getString('discord_id')) {
    if (interaction.user.id !== env.DISCORD_OWNER_ID) {
      return embedTemplate()
        .setColor(Colors.Red)
    ...
    return embedTemplate().setDescription(
      await unlink(interaction.options.getString('discord_id', true)),
    );
  }

  return embedTemplate().setDescription(await unlink(interaction.user.id));
}
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName('discord_id').setDescription('Ignore this, admin use only!'),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName('profile')
        .setDescription(
          "Show someone's TripSitLearn profile, including the courses they have completed!",
        )
        .addStringOption((option) =>
          option.setName('user').setDescription('Who are you looking up? Defaults to you.'),
        )
        .addBooleanOption((option) =>
          option
            .setName('ephemeral')
