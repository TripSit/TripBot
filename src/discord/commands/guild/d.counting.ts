...
async function handleWrongNumber(message: Message, countingDataParam: any): Promise<void> {
  if (!message.guild) return;
  if (!(message.channel instanceof TextChannel)) return;

  const countingData = countingDataParam;
  ...
  if (countingData.current_stakeholders
    ...
    const updatedStakeholders = countingData.current_stakeholders
      .split(',')
    ...
    await db.counting.upsert({
      ...
      create: { ...countingData, current_stakeholders: updatedStakeholders },
      update: { current_stakeholders: updatedStakeholders },
    });
...
export async function countMessage(message: Message): Promise<void> {
  ...
  // Use a database transaction to atomically check and update
  const result = await db.$transaction(async tx => {
    const countingData = await tx.counting.findFirst({
      where: {
        channel_id: message.channel.id,
      },
    });

    if (!countingData) return { success: false, reason: 'no_channel' };

    log.debug(F, `countingData: ${JSON.stringify(countingData, null, 2)} `);

    if (countingData.current_number === -1) {
      return { success: false, reason: 'game_starting' };
    }
    ...
    // Check token game hour limit
    if (countingData.type === 'TOKEN' && message.author.id.toString() !== env.DISCORD_OWNER_ID.toString()) {
      ...
      const lastMessage = channelMessages
        ...
        .filter(m => !Number.isNaN(parseInt(m.cleanContent, 10)))
        .filter(m => m.createdTimestamp > Date.now() - (1000 * 60 * 60))
        .sort((a, b) => b.createdTimestamp - a.createdTimestamp)
        .first();

      if (lastMessage) {
        return { success: false, reason: 'token_timeout' };
      }
    }

    // Check if this is the correct next number
    if (number !== countingData.current_number + 1) {
      return { success: false, reason: 'wrong_number', countingData };
    }

    // Update stakeholders list
    const currentStakeholders = countingData.current_stakeholders
      ? countingData.current_stakeholders.split(',')
      : [];
    if (!currentStakeholders.includes(message.author.id)) {
      currentStakeholders.push(message.author.id);
    }

    // Update the counting data atomically
    const updatedData = await tx.counting.update({
      where: {
        guild_id_channel_id: {
          guild_id: message.guild?.id ?? '',
          channel_id: message.channel.id,
      ...
      data: {
        ...
        current_number_message_id: message.id,
        current_number_message_date: message.createdAt,
        current_number_message_author: message.author.id,
        current_stakeholders: currentStakeholders.join(','),
      },
    });

    return { success: true, countingData: updatedData };
  });
  ...
  if (!result.success) {
    switch (result.reason) {
      ...
      case 'token_timeout':
        ...
        return;
      case 'wrong_number':
        await handleWrongNumber(message, result.countingData);
        return;
      default:
        return;
    }
  ...
  // Success case - the number was accepted
  log.debug(F, `Successfully counted ${number} by ${message.author.tag}`);
  await message.react('✅');

  // After successfully updating the counting data
  if (result.success && result.countingData && number % 10 === 0) {
    const stakeholders = result.countingData.current_stakeholders?.split(',') || [];
    if (stakeholders.length > 0) {
      const totalPot = calcTotalPot(number, stakeholders.length);
      const tokensPerPerson = Math.floor(totalPot / stakeholders.length);

      // Give tokens to each stakeholder using Promise.all to avoid await in loop
      await Promise.all(stakeholders.map(async stakeholderId => {
        const userData = await db.users.upsert({
          where: { discord_id: stakeholderId },
          create: { discord_id: stakeholderId },
          update: {},
        });

        await db.personas.upsert({
          where: { user_id: userData.id },
          create: { user_id: userData.id, tokens: tokensPerPerson },
          update: { tokens: { increment: tokensPerPerson } },
        });
      }));

      // Send message about the payout
      const embed = embedTemplate()
        .setTitle('Payday!')
        .setColor(Colors.Green)
        .setDescription(stripIndents`The pot of ${totalPot} tokens has been claimed by ${stakeholders.length} users!
        Each user has been given ${tokensPerPerson} tokens!
