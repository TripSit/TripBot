...
import type { SlashCommandBeta } from '../../@types/commandDef';
...
function calcTotalPot(number: number, users: number): number {
...
export async function countingReset(
  ...
  if (!countingData) {
    return { content: 'This channel is not set up for counting!' };
  }
  const number = interaction.options.getInteger('number') ?? 0;
  await countingSetup(
    channel,
    (interaction.options.getString('type') ?? 'NORMAL') as 'HARDCORE' | 'NORMAL' | 'TOKEN',
    number,
    true,
...
export async function countingScores(
  ...
  const currentLink = `[${countingData.current_number}](https://discord.com/channels/${interaction.guildId}/${interaction.channelId}/${countingData.current_number_message_id})`;
  const currentMember = await interaction.guild?.members.fetch(
    countingData.current_number_message_author,
  );
  let description = `
  **Current Combo**
  ...
  if (countingData.last_number) {
    const lastLink = `[${countingData.last_number}](https://discord.com/channels/${interaction.guildId}/${interaction.channelId}/${countingData.last_number_message_id})`;
    const lastMember = await interaction.guild?.members.fetch(
      countingData.last_number_message_author!,
    );
    const lastBreaker = await interaction.guild?.members.fetch(countingData.last_number_broken_by!);
    description += `

    **Last Combo**
    ${lastLink} - ${lastMember ?? 'unknown'} ${time(countingData.last_number_message_date!, 'R')}
    Broken by ${lastBreaker ?? 'unknown'} ${time(countingData.last_number_broken_date!, 'R')}`;
  }

  if (countingData.record_number) {
    const recordLink = `[${countingData.record_number}](https://discord.com/channels/${interaction.guildId}/${interaction.channelId}/${countingData.record_number_message_id})`;
    const recordMember = await interaction.guild?.members.fetch(
      countingData.record_number_message_author!,
    );
    const recordBreaker = await interaction.guild?.members.fetch(
      countingData.record_number_broken_by!,
    );
    description += `

    **Record Combo**
    ${recordLink} - ${recordMember ?? 'unknown'} ${time(countingData.record_number_message_date!, 'R')}
    Broken by ${recordBreaker ?? 'unknown'} ${time(countingData.record_number_broken_date!, 'R')}`;
  }
...
export async function countingSetup(
  ...
  if (countingData && !override) {
    return {
      embeds: [
        embed.setTitle('This channel is already set up for counting! Mods can /reset or /end it!'),
      ],
    };
  }
  ...
  await db.counting.upsert({
    ...
    update: {
      ...
      current_number_message_id: countingMessage.id,
      current_stakeholders: currentStakeholders,
      last_number: countingData?.last_number ? countingData.last_number : null,
...
export async function countMessage(message: Message): Promise<void> {
  ...
  if (!countingData) {
    return;
  } // If not a counting channel then ignore all messages
  ...
  const number = Number.parseInt(message.cleanContent, 10);
  ...
  if (
    ...
    // Allow the owner to spam (for testing)
    log.debug(
      F,
      'Deleting message because the author is the same as the current number message author',
    );
  ...
  const lastMessage = channelMessages
    ...
    .filter((m) => !Number.isNaN(Number.parseInt(m.cleanContent, 10))) // That are numbers
  ...
  if (number !== countingData.current_number + 1) {
    ...
    if (
      countingData.current_stakeholders &&
      ...
      if (countingData.type === 'HARDCORE') {
        ...
        messageReply += `If you break the combo again, you'll steal ${totalPot} tokens and reset the counter for everyone else!`;
      } else {
    ...
    if (countingData.current_number > countingData.record_number) {
      ...
      const recordUser = await message.guild.members.fetch(
        countingData.record_number_message_author,
      );
    ...
    await message.channel.send({
      embeds: [
        embedTemplate()
          ...
          .setColor(Colors.Red)
          .setDescription(
            stripIndents`Oh no, ${message.author} broke the combo...${recordMessage}${endingMessage}`,
          ),
      ],
    });
    ...
    const resetTime = new Date(Date.now() + 10 * 1000);
    const newMessage = await message.channel.send(`Starting new game ${time(resetTime, 'R')}`);
    await sleep(10_000);

    // Start a new counting game
    await countingSetup(message.channel, countingData.type, 0, true, true);
  ...
  if (countingData.current_stakeholders) {
    if (countingData.current_stakeholders.includes(message.author.id)) {
      ...
      if (countingData.type === 'TOKEN') {
        welcomeMessage +=
          '\nThis is a TOKEN game: Build the pot and get tokens every 10 levels, or break the combo and steal it all!';
  ...
  if (number % 10 === 0 && countingData.type === 'TOKEN') {
    ...
    await Promise.all(
      stakeholderIds.map(async (discordId) => {
        // const userData = await getUser(message.author.id, null, null);
        const userData = await db.users.upsert({
          create: {
    ...
    const embed = embedTemplate()
      ...
      .setDescription(
        stripIndents`The pot of ${totalPot} tokens has been claimed by ${stakeholderNumber} users!
...
export const counting: SlashCommandBeta = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option.setDescription('The number to set the channel to').setName('number'),
        )
        .addBooleanOption((option) =>
          option.setName('purge').setDescription('Set to "True" to start completely fresh'),
        )
        .addStringOption((option) =>
          option
            .setDescription('What kind of counting game?')
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('end').setDescription('End the counting game!'),
    ),
  async execute(interaction) {
    ...
    let response = {
      content: 'This command has not been setup yet!',
    } as InteractionEditReplyOptions;
    if (command === 'setup') {
      // Check if the user can manage the channel role
      if (
        !(await checkChannelPermissions(interaction.channel as TextChannel, [
