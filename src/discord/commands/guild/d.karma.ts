...
export const dKarma: SlashCommand = {
  ...
  async execute(interaction) {
    ...
    const message = `${member.displayName} has received ${userData.karma_received} karma and given ${userData.karma_given} karma`;

    // const quote = karmaQuotes[Math.floor(Math.random() * karmaQuotes.length)];
    const embed = embedTemplate().setTitle(message);
