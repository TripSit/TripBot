...
export const dLeaderboard: SlashCommand = {
  ...
  async execute(interaction) { // eslint-disable-line
    ...
    const categoryName =
      categoryChoices.find((choice) => choice.value === categoryChoice)?.name || 'Total';

    context.font = resizeText(canvasObject, categoryName.toUpperCase(), 40, 'futura', 498);
    context.fillText(categoryName.toUpperCase(), 276, 54);

    // UPDATE THIS WHEN TIME PERIOD IS ADDED
    const timePeriod = 'ALL TIME';
    context.font = resizeText(canvasObject, `TOP OF ${timePeriod}`, 40, 'futura', 498);
        ...
        while (count < 10) {
          ...
          if (user) {
            ...
            if (member) {
              ...
              const roleColor = `#${(member.roles.color?.color || 0x99_aa_b5).toString(16).padStart(6, '0')}`;
              ...
              if (personaData) {
                ...
                const equippedFont = inventoryData.find(
                  (item) => item.equipped && item.effect === 'font',
                );
                if (equippedFont) {
              ...
              context.drawImage(
                avatar,
                bar.x + avatarOffset - bar.height / 2,
                bar.y,
                bar.height,
                bar.height,
              );
              ...
              const maxLength =
                bar.width - (levelTextWidth + 18) - (18 + avatarOffset + bar.height / 2);
              context.font = `${fontSize}px ${userFont}`;
              context.fillStyle = userNameColor;
              context.font = resizeText(canvasObject, userName, fontSize, userFont, maxLength);
              context.textAlign = 'left';
              context.fillText(
                userName,
                bar.x + avatarOffset + bar.height / 2 + 9,
                bar.y + bar.height / 2,
              );
