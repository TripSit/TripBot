...
type VoiceActions = 'add' | 'ban' | 'host' | 'level' | 'limit' | 'lock' | 'name' | 'ping';
...
async function tentAdd(
  ...
  if (
    hasExplicitPermission(voiceChannel, target, PermissionsBitField.Flags.ViewChannel) === false
  ) {
  ...
  return embedTemplate().setTitle(title).setColor(Colors.Green).setDescription(description);
}
...
async function tentBan(
  ...
  if (
    ...
    hasExplicitPermission(voiceChannel, target, PermissionsBitField.Flags.MoveMembers) === true
  ) {
...
async function tentHost(
  ...
  if (newHost && newHost.voice.channel === voiceChannel) {
    ...
    return embedTemplate().setTitle('User not connected').setColor(Colors.Red)
  ...
  const embed = new EmbedBuilder().setTitle('Tent updated').setColor(Colors.Blue).setDescription(`
    ${newHost} is now host.
...
async function tentLevel(
  ...
  const level = Number.parseInt(levelNumber, 10);
  let title = '';
  let description = '';
  ...
  if (level === 0) {
    ...
    for (const key of Object.keys(levelRoles)) {
      const roleLevel = Number.parseInt(key, 10);
      if (roleLevel < level) {
...
async function tentLimit(voiceChannel: VoiceBasedChannel, limit: number): Promise<EmbedBuilder> {
  let title = '';
  let description = '';
  if (limit === 0) {
...
async function tentLock(voiceChannel: VoiceBasedChannel): Promise<EmbedBuilder> {
  let title = '';
  let description = '';
  if (
    voiceChannel
...
let lastTentPingTime = Date.now() - 3_600_000; // Initialize to one hour ago
const userTentPingTimes: Record<string, number> = {}; // Initialize an empty object to store user ping times
const globalCooldown = 3_600_000; // 1 hour
const userCooldown = 10_800_000; // 3 hours
...
async function tentPing(
  ...
  if (role) {
    ...
    if (userTentPingTimes[userId] && now - userTentPingTimes[userId] < userCooldown) {
      return embedTemplate().setTitle('Cooldown').setColor(Colors.Red)
        .setDescription(`You already used this command <t:${Math.floor(userTentPingTimes[userId] / 1000).toString()}:R>.
        You can use it again <t:${Math.floor((userTentPingTimes[userId] + userCooldown) / 1000).toString()}:R>.`);
    }
  ...
  return embedTemplate()
    ...
    .setColor(Colors.Red)
    .setDescription(
      'The Join VC role could not be found. This should not have happened, please contact a developer.',
    );
}
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName('target').setDescription('The user to transfer host to').setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('lock').setDescription('Lock/Unlock your Tent'),
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName('level')
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName('target').setDescription('The user to add/unadd').setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName('target').setDescription('The user to ban/unban').setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('ping').setDescription('Ping the Join VC role'),
    ),
