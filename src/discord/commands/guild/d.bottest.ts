...
async function runCommand(
  ...
  const testInteraction = {
    ...
    editReply: async (content: string) => interaction.followUp(content),
    followUp: async (content: string) => interaction.followUp(content),
    guild: interaction.guild,
    member: interaction.member,
    options: {},
    reply: async (content: string) => interaction.followUp(content),
    user: interaction.user,
  };

  // log.debug(F, `Running command: ${name}`);

  if (!testableCommands.includes(commandName)) {
    return null;
  }
  ...
  if (command) {
    ...
    if (commandName === 'birthday') {
      ...
      await interaction.channel.send(
        `> **${commandName}** - Setting new birthdate to ${monthName} ${day}`,
      );
      testInteraction.options = {
        getBoolean: (name: string) => {
          if (name === 'ephemeral') {
      ...
      await interaction.channel.send(
        `> **${commandName}** - Getting new birthdate (Should be ${monthName} ${day})`,
      );
      testInteraction.options = {
        getBoolean: (name: string) => {
          if (name === 'ephemeral') {
    ...
    if (commandName === 'dramacounter') {
      ...
      testInteraction.options = {
        ...
        getString: (name: string) => {
          if (name === 'dramatime') {
            return `${randomValue} hours ago`;
          }
          if (name === 'dramaissue') {
            return `Testing ${randomValue}`;
          }
          return null;
        },
        getSubcommand: () => 'set',
      ...
      // Test getting the existing drama
      await interaction.channel.send(
        `> **${commandName}** - Get new record, should be the same as above`,
      );
      testInteraction.options = {
        getBoolean: (name: string) => {
          if (name === 'ephemeral') {
    ...
    if (commandName === 'poll') {
      testInteraction.options = {
        ...
        getString: (name: string) => {
          if (name === 'question') {
            return 'Is TripBot Awesome?';
          }
          if (name === 'choices') {
            return 'Yes,Also Yes,No...but yes';
          }
          return null;
        },
      };
    ...
    if (commandName === 'reminder') {
      testInteraction.channel = (await interaction.guild?.channels.fetch(
    ...
    if (commandName === 'timezone') {
      ...
      // Set the record
      await interaction.channel.send(
        `> **${commandName}** - Setting new timezone to 'America/Chicago'`,
      );
      testInteraction.options = {
        getBoolean: (name: string) => {
          if (name === 'ephemeral') {
      ...
      // Get the new record
      await interaction.channel.send(
        `> **${commandName}** - Getting new record (Should be same as above)`,
      );
      testInteraction.options = {
        getBoolean: (name: string) => {
          if (name === 'ephemeral') {
      ...
      // Set the record
      await interaction.channel.send(
        `> **${commandName}** - Setting new timezone to 'America/New_York'`,
      );
      testInteraction.options = {
        getBoolean: (name: string) => {
          if (name === 'ephemeral') {
    ...
    if (commandName === 'youtube') {
      testInteraction.options = {
        ...
        getString: (name: string) => {
          if (name === 'search') {
            return 'TripSit find the others';
          }
          return null;
        },
      };
    ...
    await discordClient.application?.commands
      ...
      .then(async (globalCommands) => {
        ...
        for (const command of globalCommands) {
          ...
          await runCommand(interaction, command[1].name).then((result) => {
            switch (result) {
              case false: {
...
async function testGuild(
  ...
  await interaction.guild.commands.fetch({ force: true }).then(async (guildCommands) => {
...
export const dBottest: SlashCommand = {
  ...
  async execute(interaction) {
    ...
    await testGlobal(interaction, results).then(async (globalResults) => {
      // log.debug(F, `Global results: ${JSON.stringify(globalResults)}`);
      await testGuild(interaction, globalResults).then(async (guildResults) => {
        if (!interaction.channel) {
          await interaction.editReply('This command must be used in a channel!');
          return false;
        }
        ...
        const embed = embedTemplate()
          ...
          .setDescription(
            guildResults.failed.length > 0
              ? `The following commands failed testing: ${guildResults.failed.join(', ')}`
              : 'All commands passed testing!',
          )
          .addFields(
            { inline: true, name: 'Tested', value: `${guildResults.total.length}` },
            { inline: true, name: 'Success', value: `${guildResults.passed.length}` },
            { inline: true, name: 'Failed', value: `${guildResults.failed.length}` },
          );
