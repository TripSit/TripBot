...
async function deploy(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (channelTripbot) {
    await channelTripbot.send(
      `${interaction.member} triggered a command deploy <@${env.DISCORD_OWNER_ID}>.`,
    );
  }
...
async function overwriteUserData(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (!experienceData) {
    log.debug(
      F,
      `No experience data found for user ${userData.id} in category ${category} type ${type}.`,
    );
  ...
  log.debug(
    F,
    `Overwriting user data for user ${userData.id} in category ${category} type ${type} to level ${level} with ${levelPoints} XP points.`,
  );
  ...
  await interaction.editReply(
    `User level and points updated for category ${category} to level ${level} with ${levelPoints} points.`,
  );
}

async function rebuild(interaction: ChatInputCommandInteraction): Promise<void> {
  log.info(F, 'Rebuilding');
  await interaction.editReply('Rebuilding');
  const channelTripbot = interaction.guild?.channels.cache.get(env.CHANNEL_BOTLOG) as TextChannel;
  if (channelTripbot) {
    await channelTripbot.send(
      `${interaction.member} triggered a rebuild <@${env.DISCORD_OWNER_ID}>.`,
    );
  }

  axios
    .post(
      'https://drone.tripsit.me/api/repos/TripSit/TripBot/builds',
      {},
      {
...
async function restart(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (channelTripbot) {
    await channelTripbot.send(
      `${interaction.member} restarted the bot <@${env.DISCORD_OWNER_ID}>.`,
    );
  }
...
export const dAdmin: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => subcommand.setName('rebuild').setDescription('Rebuild the bot'))
    .addSubcommand((subcommand) => subcommand.setName('deploy').setDescription('Deploy commands'))
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName('status').setDescription('The status text').setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName('url').setDescription('The URL of the avatar').setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName('url').setDescription('The URL of the banner').setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName('type').setDescription('The type to update').setRequired(true).addChoices(
  ...
  async execute(interaction) {
    ...
    switch (command) {
      ...
      case 'setavatar': {
        await setAvatar(interaction, interaction.options.getString('url')!);
        break;
      }
      case 'setbanner': {
        await setBanner(interaction, interaction.options.getString('url')!);
        break;
      }
      case 'setstatus': {
        await setStatus(
