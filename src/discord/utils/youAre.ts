...
export function valMatch(input: string, regex: RegExp, expLength: number) {
...
export async function youAre(message: Message): Promise<void> {
  ...
  if (message.channel.parentId === env.CATEGORY_HARMREDUCTIONCENTRE) {
    return;
  }
  ...
  if (response.isFlaggedOrLowConfidence) {
    log.info(
      F,
      `Message flagged by ModerateHatespeech API: "You're ${phrase}\n ${response.class} (${response.confidence})"`,
    );
    return;
  }
