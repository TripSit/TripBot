...
export async function techHelpClick(interaction: ButtonInteraction) {
  ...
  const filter = (index: ModalSubmitInteraction) => index.customId.includes('techHelpSubmit');
  interaction.awaitModalSubmit({ filter, time: 0 }).then(async (index) => {
    ...
    const modalInput = index.fields.getTextInputValue(`${issueType}IssueInput`);
    // log.debug(F, `modalInput: ${modalInput}!`);

    // Get the actor
    const actor = (index.member ?? index.user) as GuildMember | User;

    // Get the actors name.
    const targetName =
      (actor as GuildMember | null)?.displayName ??
      (actor as null | User)?.username ??
      '[Deleted Discord Account]';

    // Create a new thread in channel
    const ticketThread = await (index.channel as TextChannel).threads.create({
    ...
    const techHelpButtons = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
    ...
    await ticketThread.send({ components: [techHelpButtons], content: message });
...
export async function techHelpClose(interaction: ButtonInteraction) {
  ...
  if (interaction.member === target) {
    ...
    (interaction.channel as ThreadChannel).setName(
      `💚${(interaction.channel as ThreadChannel).name.slice(1)}`,
    );
  } else {
    (interaction.channel as ThreadChannel).setName(
      `💙${(interaction.channel as ThreadChannel).name.slice(1)}`,
  ...
  await interaction.reply({
    embeds: [new EmbedBuilder().setDescription(message).setColor(Colors.Green)],
  });
}
...
export async function techHelpOwn(interaction: ButtonInteraction) {
  ...
  await interaction.reply({
    embeds: [
      new EmbedBuilder()
        .setDescription(
          stripIndents`${(interaction.member as GuildMember).displayName} has claimed this \
