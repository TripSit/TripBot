...
const rejectionMessages = {
  ...
  enabling:
    'we have found in your personal user history where you have directly advocated harmful practices. This is easy to do when you get carried away, and we understand that drug use is fun and not always to be taken seriously, but we have reservations for this reason. This can always change, though, over time!',
  exposure:
  ...
  overstaffed:
    "we currently have too many resources of that of that current position and we don't want to have a 'too many cooks' situation. We will keep your application and review again in the near future.",
  tooNew:
    "your Discord account is too new. Let's get to know each other for a while, eh? To be transparent, the minimum account age for our helpers is 1 month(s) (subject to change) for consideration. We will review again in the future.",
  young:
...
export async function applicationApprove(interaction: ButtonInteraction): Promise<void> {
  ...
  if (!(await applicationPermissions(interaction, null, interaction.channel as TextChannel))) {
    return;
  }
  ...
  if (threadCreated && threadCreated.getTime() > Date.now() - 86_400_000) {
    // log.debug(F, `Thread created in the last 24 hours!`);
    await interaction.editReply({
      content:
        'Whoa there, please give the team at least 24 until the next day to act on this application!',
    });
    return;
  }
  ...
  (interaction.channel as ThreadChannel).setName(
    `💚│${target.displayName}'s ${role.name} application1!`,
  );

  if (role.id === env.ROLE_HELPER) {
    const channelTripsitmeta = (await interaction.guild?.channels.fetch(
  ...
  if (role.id === env.ROLE_CONTRIBUTOR) {
    ...
    const channelTripmobile = (await interaction.guild?.channels.fetch(
      env.CHANNEL_TRIPMOBILE,
    )) as TextChannel;
    const channelContent = (await interaction.guild?.channels.fetch(
      env.CHANNEL_CONTENT,
    )) as TextChannel;
    const channelDevelopment = (await interaction.guild?.channels.fetch(
      env.CHANNEL_DEVELOPMENT,
    )) as TextChannel;
...
export async function applicationPermissions(
  interaction: ButtonInteraction | ChatInputCommandInteraction | SelectMenuInteraction,
  applicationPostChannel: null | TextChannel,
  ...
  if (!interaction.member) {
    ...
    await interaction.reply({
      content: 'This must be performed by a member of a guild!',
      flags: MessageFlags.Ephemeral,
    });
    return false;
  }
  if (!interaction.channel) {
    // log.debug(F, `no member!`);
    await interaction.reply({
      content: 'This must be performed inside of a channel!',
      flags: MessageFlags.Ephemeral,
    });
    return false;
  }
  if (interaction.channel.type !== ChannelType.GuildText) {
    // log.debug(F, `no member!`);
    await interaction.reply({
      content: 'This must be performed inside of a text channel!',
      flags: MessageFlags.Ephemeral,
    });
    return false;
  }
  ...
  if (applicationPostChannel) {
    ...
    if (!channelPerms.hasPermission) {
      log.error(
        F,
        `Missing TS channel permission ${channelPerms.permission} in ${applicationPostChannel}!`,
      );
      await interaction.reply({
  ...
  const metaChannelPerms = await checkChannelPermissions(applicationThreadChannel, [
    'ViewChannel' as PermissionResolvable,
    'SendMessages' as PermissionResolvable,
  ...
  if (!metaChannelPerms.hasPermission) {
    log.error(
      F,
      `Missing TS channel permission ${metaChannelPerms.permission} in ${applicationThreadChannel}!`,
    );
    await interaction.reply({
...
export async function applicationReject(interaction: SelectMenuInteraction): Promise<void> {
  log.info(F, await commandContext(interaction));
  if (!interaction.guild) {
    return;
  }
  ...
  await interaction.editReply(
    `${actor.displayName} rejected this application with reason code '${rejectionReason}'`,
  );
  ...
  (interaction.channel as ThreadChannel).setName(
    `🖤│${target.displayName}'s ${role.name} application!`,
  );
}

export async function applicationSetup(interaction: ChatInputCommandInteraction) {
  if (!interaction.guild) {
    return;
  }
  if (!interaction.channel) {
      ...
      .addComponents(
        new ActionRowBuilder<TextInputBuilder>().addComponents(
          new TextInputBuilder()
            ...
            .setStyle(TextInputStyle.Paragraph).setValue(stripIndents`
  ...
  const filter = (index: ModalSubmitInteraction) => index.customId.startsWith('appModal');
    ...
    roleArray.forEach(async (role) => {
      if (role[0]) {
        if (role[1]) {
          ...
          await index.reply('Error: You must provide both a role and a reviewer role!');
        }
      }
    });

    await (index.channel as TextChannel).send({
      components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(selectMenu)],
      content: stripIndents`${index.fields.getTextInputValue('applicationText')}`,
    });
    await index.editReply({ content: 'Donezo!' });
  });
}

export async function applicationStart(interaction: SelectMenuInteraction): Promise<void> {
  log.info(F, await commandContext(interaction));
  ...
  const channelApplications = (await interaction.guild.channels.fetch(
    channelApplicationsId,
  )) as TextChannel;
      ...
      .addComponents(
        ...
        new ActionRowBuilder<TextInputBuilder>().addComponents(
          new TextInputBuilder()
            ...
            .setPlaceholder(
              `What makes you qualified to be a ${roleRequested.name}? What can you bring to the team?`,
            )
            .setMaxLength(2000)
            .setStyle(TextInputStyle.Paragraph),
        ),
  ...
  const filter = (index: ModalSubmitInteraction) => index.customId.startsWith('applicationSubmit');
  interaction.awaitModalSubmit({ filter, time: 0 }).then(async (index) => {
    if (index.customId.split('~')[1] !== interaction.id) {
      return;
    }
    ...
    const actor = index.member as GuildMember;

    const reason = index.fields.getTextInputValue('reason');
    log.debug(F, `reason.length: ${reason.length}`);
    const skills = index.fields.getTextInputValue('skills');
    ...
    const appEmbed = embedTemplate()
      ...
      .addFields(
        ...
        },
        {
          inline: true,
          name: 'Username',
          value: `${index.member.user.username}#${index.member.user.discriminator}`,
        },
        {
          inline: true,
          name: 'ID',
          value: index.member.user.id,
        },
      )
      .addFields({
        inline: true,
        name: 'Created',
        value: time((index.member.user as User).createdAt, 'R'),
      });
    if (actor.joinedAt) {
      columns = 2;
      appEmbed.addFields({
    ...
    appEmbed.addFields({ inline: true, name: '\u200B', value: '\u200B' });
    ...
    await applicationThread.send(
      `Hey ${roleReviewer} there is a new ${roleRequested.name} application from ${actor.displayName}!`,
    );
    await applicationThread
      .send({
        allowedMentions: {
          // parse: showMentions,
    ...
    const embed = embedTemplate()
      .setColor(Colors.DarkBlue)
      .setDescription(
        'Thank you for your interest! We will try to get back to you as soon as possible!',
      );
