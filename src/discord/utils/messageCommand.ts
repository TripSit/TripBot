...
import { DMChannel, EmbedBuilder, Role, TextChannel } from 'discord.js';
...
export async function messageCommand(message: Message): Promise<void> {
  ...
  if (await isIrcCommand(message)) {
    ...
    if (command === 'tripsit') {
      ...
      if (
        !tripsitChannels.has(message.channel.id) &&
        !message.channel.name.endsWith(`${message.author.displayName}'s channel!`)
      ) {
        const channelTripsit = (await message.guild.channels.fetch(
          env.CHANNEL_TRIPSIT,
        )) as TextChannel;
        const channelOpenTripsit1 = (await message.guild.channels.fetch(
          env.CHANNEL_OPENTRIPSIT1,
        )) as TextChannel;
      ...
      const roleTripsitter = (await message.guild.roles.fetch(env.ROLE_TRIPSITTER))!;
      const roleHelper = (await message.guild.roles.fetch(env.ROLE_HELPER))!;
      await message.channel.send({
        allowedMentions: {
      ...
      **If this is a medical emergency** please contact your local emergency services: we do not call EMS on behalf of anyone.
      ...
      const embed = new EmbedBuilder().setTitle('Other Resources')
    ...
    if (
      (await isBotOwner(message)) &&
      message.content.toLowerCase().includes('tokens') &&
      message.content.toLowerCase().includes('give')
    ) {
      // use regex to find the number in the message.cleanContent
      const amount = Number.parseInt(/(\d+)/.exec(message.cleanContent)?.[0] ?? '0', 10);
      ...
      recipients.forEach(async (recipient) => {
        ...
        if (
          !recipientMember &&
          (message.channel instanceof TextChannel || message.channel instanceof DMChannel)
        ) {
        ...
        log.debug(
          F,
          `Gave ${amount} tokens to ${recipientMember ? recipientMember.displayName : recipient.username}!`,
        );
        if (message.channel instanceof TextChannel || message.channel instanceof DMChannel) {
          await message.channel
            .send(stripIndents`Gave ${amount} tokens to ${recipientMember ? recipientMember.displayName : recipient.username}!
    ...
    if ((await isBotOwner(message)) && message.content.toLowerCase().includes('phoenix')) {
      ...
      await sleep(1000);
      await phoenixMessage.edit(
        `Phoenix protocol deployed. Good luck ${message.member?.displayName} <3`,
      );
      await sleep(3000);
    ...
    if (
      (await isUploadMessage(message)) &&
      message.member?.permissions.has('ManageEmojisAndStickers' as PermissionResolvable)
    ) {
      if (message.content.toLowerCase().includes('emoji')) {
        ...
        if (!emojis && message.reference?.messageId) {
        ...
        if (emojis) {
          ...
          for (const emoji of emojis) { // eslint-disable-line
            ...
            try {
              ...
              emojiSuccessList.push(
                `<${emojiAnimated ? 'a' : ''}:${emojiData.name}:${emojiData.id}>`,
              );
            } catch (error) {
              log.error(F, `Error creating emoji: ${error}`);
      ...
      if (message.content.toLowerCase().includes('sticker')) {
        await message.channel.send(stripIndents`Hey ${displayName}, uploading emojis...`);
        ...
        if (message.stickers) {
          ...
          await message.channel.send(`Uploaded ${stickerList.join(' ')} to ${message.guild.name}!`);
        }
      }
      ...
      try {
        ...
        log.error(F, `Error reacting to message: ${error}`);
        try {
          await message.react('💜');
        } catch {
...
async function isMentioningTripbot(message: Message): Promise<boolean> {
  return (
    ...
    message.mentions.roles.has(env.ROLE_TRIPBOT)
  );
}
...
async function messageContainsHearts(message: Message): Promise<boolean> {
  return heartEmojis.some(
    (word) =>
      message.cleanContent.includes(word) &&
      !message.cleanContent.slice(Math.max(0, message.cleanContent.indexOf(':') + 1)).includes(':'),
  );
}
