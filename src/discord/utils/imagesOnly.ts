...
export async function imagesOnly(message: Message): Promise<void> {
  ...
  if (message.channel.id !== env.CHANNEL_MEMES) return; // Only do this in the memes channel

  const member = await message.guild.members.fetch(message.author.id);
  if (member.roles.cache.has(env.ROLE_MODERATOR)) return; // If mod then ignore

  if (message.embeds.length === 0 && message.attachments.size === 0) message.delete();
}
