/**
 * Reference dump: persisted/DB-backed Tent settings system, ported from the
 * `Tent-Moderation-Overhaul` branch (last active ~Feb 2025, branched off `main`
 * long before the current `tent_settings`/`tent_hostlist`/`tent_whitelist`/
 * `tent_blacklist`/`tent_publicity` schema landed on TentUpdate).
 *
 * This file is NOT wired into the bot. It is not imported anywhere, exports
 * nothing live, and the command loader (src/discord/commands/index.ts) does
 * not scan this directory, so it has zero runtime effect. It exists purely so
 * the substance of that branch's work survives after the branch is deleted.
 *
 * What this is: a full rewrite of `d.voice.ts` (the `/tent` command) that
 * replaces today's live-permission-overwrite model with a DB-persisted one:
 *   - `tent_settings` row per tent (mode, join_level, name), survives channel
 *     teardown/recreate
 *   - `tent_hostlist` / `tent_whitelist` / `tent_blacklist` join tables for
 *     per-tent host/allow/ban lists (today's version encodes all of this as
 *     live Discord permission overwrites only)
 *   - A button/select-menu/modal driven `/voice settings` GUI for managing
 *     the above, dispatched via `voiceButton` / `voiceSelect` exports
 *   - `voiceUpdate()` consolidates what `src/discord/utils/tents.ts`
 *     (pitchTent/teardownTent/logTent/updateTentStatus) does today
 *
 * Adaptations already made so it lines up with TentUpdate's actual schema:
 *   - `users.joinTentLevel`  -> `users.defaultTentJoinLevel`
 *   (the old branch's schema used bare `banlist`/`whitelist`/`joinTentLevel`
 *   on `users`; TentUpdate already renamed these to `defaultTentBanlist` /
 *   `defaultTentWhitelist` / `defaultTentJoinLevel` for clarity and fixed
 *   `tent_blacklist`'s relation field from `channel` to `tent` -- that field
 *   name itself is never referenced by name in this file's Prisma calls, so
 *   no further edit was needed for it.)
 *
 * NOT adapted / known gaps before this could be enabled for real:
 *   - Still references `src/discord/utils/tents.ts` conceptually but doesn't
 *     import it -- current `tents.ts` on `main`/TentUpdate grew independently
 *     (~330 lines) since this branch forked, so this is NOT a drop-in
 *     replacement. Reconciling the two needs a real read of both.
 *   - Open bug noted in the original (never fixed): nothing stops a user
 *     from being on both a tent's whitelist and blacklist at once -- see the
 *     TODO a few lines down.
 *   - Migrations from that branch are NOT included here -- they were all
 *     misnamed `..._welcome_fix` and don't match TentUpdate's schema anyway
 *     (field names differ). A fresh migration will be needed once/if this is
 *     built out for real.
 *   - Not type-checked or lint-checked against the current codebase; treat
 *     every line below as a starting sketch, not working code.
 *
 * Source: git branch origin/Tent-Moderation-Overhaul, file
 * src/discord/commands/guild/d.voice.ts (2098 lines at HEAD 84c3a0f2).
 * The buttonClick.ts / selectMenu.ts dispatch hooks this file's `voiceButton`
 * / `voiceSelect` need are sketched as comments at their would-be call sites
 * in those two files.
 */
...
//           join_level: userData.defaultTentJoinLevel,
//         },
//       });
//     }
//
...
//     const actor = interaction.member as GuildMember;
//
//     // Also update defaultTentJoinLevel in the users saved settings
//     await db.users.upsert({
//       where: { discord_id: actor.id },
//       create: { discord_id: actor.id },
//       update: { defaultTentJoinLevel: level },
//     });
//
