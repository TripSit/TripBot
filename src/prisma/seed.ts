import type { drug_roa, drug_variants, users } from '@prisma/client';
...
import combinedDatabase from '../../assets/data/combinedDB.json';

const drugs = combinedDatabase as [CallbackSubstance];

type DrugRecord = CallbackSubstance & { id: string };
...
function parseDuration(name: Period, range: string, durations: Duration[] | null) {
  ...
  if (!/\d/.test(valueText)) {
    return null;
  } // Validation to omit dirty values
...
const parseDose = (name: Strength, doses: Dosage[] | null) => {
  ...
  let value = Number.parseFloat(dose.value.split('-')[0].replaceAll(/\D/g, ''));
      ...
      for (const roa of drug.roas.filter((r) => r.name)) {
        await prisma.drug_variant_roas.create({
          data: {
            ...
            duration_after_effects_max: parseDuration(
              'after effects' as Period,
              'max',
              roa.duration ?? null,
            ),

            duration_after_effects_min: parseDuration(
              'after effects' as Period,
              'min',
              roa.duration ?? null,
            ),
            ...
            route: (
              routeMap[roa.name.toLowerCase() as keyof typeof routeMap] ?? roa.name
            ).toUpperCase() as drug_roa,
          },
        });
      }
    }
...
async function seedDrugVariants(
  drugRecords: DrugRecord[],
  userId: string,
): Promise<drug_variants[]> {
  return await Promise.all(
    drugRecords.map(
      async (drug) =>
        await prisma.drug_variants.create({
          data: {
            default: true,
            drug_id: drug.id,
            last_updated_by: userId,
...
async function seedExperience(userList: users[]): Promise<void> {
