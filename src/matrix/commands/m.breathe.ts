...
async function mBreathe(roomId:string, event:any, client:MatrixClient, choice:string | null):Promise<boolean> {
  const url = await breathe(choice);
  client.replyNotice(roomId, event, `Click on the image below:\n${url}`);
  return true;
}
