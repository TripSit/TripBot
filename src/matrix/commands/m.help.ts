...
export default async function mHelp(roomId: string, event: any, client: MatrixClient): Promise<boolean> {
  ...
  await client.sendMessage(roomId, RichReply.createFor(roomId, event, text, html));
  return true;
}
